...
- 示例 `docker-compose.yml` 已内置可直接运行的默认值（端口 8080、库名 `hive_db`、密码 `123456` 等），
  开箱即用；**生产环境务必改掉密码和 `HIVE_JWT_SECRET`**（直接改 compose 里对应的值即可）。
- 端口、数据库地址、密码、JWT 密钥等底层仍由环境变量驱动；`application.yml` 保留 localhost 默认值，
  本地开发方式不受影响。
